...
if [ "$node_id" = "1" ]; then
  ...
  mkdir /tmp/etcd
  curl -L --silent $etcd_url -o /tmp/etcd/etcd-v${etcd_version}-linux-amd64.tar.gz 
  cd /tmp/etcd && tar xzvf etcd-v${etcd_version}-linux-amd64.tar.gz
  cd /tmp/etcd && nohup etcd-v${etcd_version}-linux-amd64/etcd --advertise-client-urls=http://${primary_ip}:2379 --listen-client-urls=http://${primary_ip}:2379 > etcd.log &
  sleep 10
fi
